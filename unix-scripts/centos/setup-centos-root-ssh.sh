...
function setupSSH {
	if [ $TOTAL_NODES == 1 ]
	then
		echo "Skipping root ssh setup for single node"
		return 0
	fi
	echo "modifying root ssh authorized key files"
	for i in $(seq 1 $TOTAL_NODES)
	do
	    sshpass -p vagrant ssh-copy-id -o StrictHostKeyChecking=no root@$NODE_NAME${i}
	done
	...
	do
		...
		then
			 ...
			 do
				 echo sshpass -p vagrant ssh-copy-id -o StrictHostKeyChecking=no root@${NODE_NAME}${j} >> /tmp/sshbypass.sh
			 done
